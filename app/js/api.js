...
const LocalCoreInstance = LocalCore.create(LocalDB, typeof LocalCache !== 'undefined' ? LocalCache.create() : null);
...
function withLock(fn) {
  // Chain on the previous operation (catch swallows errors so the queue keeps moving)
...
const LocalAPI = {
  ...
  async route(method, url, body) {
    ...
    try {
      // GET requests bypass the write queue for better read concurrency
