// Settings module - profile + multi-provider API config
const Settings = {
  presets: [],
  providerId: null,
  provider: null,
  ...
  async renderDetail(id) {
    ...
    try {
      ...
      document.getElementById('p-key-hint').textContent = detail.key_configured
        ? `已配置 (${detail.key_masked})，点“显示”可查看`
        : '尚未配置';
  ...
  async saveProvider(silent = false) {
    ...
    try {
      ...
      document.getElementById('p-key-hint').textContent = key
        ? `已配置 (****${key.slice(-4)})，点“显示”可查看`
        : document.getElementById('p-key-hint').textContent;
      if (!silent) Toast.show('配置已保存，密钥仍保留在输入框中', 'success');
      return { ok: true };
    } catch (e) {
