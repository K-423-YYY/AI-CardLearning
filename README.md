...
- AI 加速：按文件大小自动选择普通 / 5倍 / 10倍 / 20倍档位，阈值支持 KB/MB 自定义，并在分析与生成时显示当前档位
...
- Windows 电脑（推荐）：下载并安装 **`AI.Setup.0.1.5.exe`**
- Windows 免安装版：下载 **`AI.0.1.5.exe`** 或 **`AI.exe`**，解压/保存后双击打开
...
单元测试（21 项）覆盖学习区、每日队列、答题判定、错题、复习排期、关卡排版、批量操作、导入导出、设置加密、本地 AI 文件筛选/批量生成、试卷识别与 Word 解析；浏览器端到端测试覆盖手机/桌面布局、Toast 自动消失、离线可用、备份导出和示例备份导入。
